...
df['PREDICTED'] = gb_model.predict(X)
...
df.to_csv("fao_output_gb_with_flags.csv", index=False)
print("Saved output → fao_output_gb_with_flags.csv")
...
plt.title("FAO Crop Production Monitoring using Gradient Boosting")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
